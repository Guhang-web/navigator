...
const callData = {
  ...
  schedule_not_fit: {
    ...
    questions: [
      "지금 꼭 시작해야 하는 이유가 있는지",
      "다음 개강까지 기다릴 수 있는지 확인",
      "비대면 수업 가능 여부 확인"
    ],
    scripts: [
      "맞아요. 시간표가 안 맞으면 억지로 시작하는 건 저도 추천드리진 않아요.",
      "다만 OOO님처럼 빨리 시작하고 싶으신 분들은 현재 가능한 반 중에서 우선 시작 가능한 방향을 보고, 정확히 원하는 시간대는 다음 개강이나 비대면 수업으로 이어가는 방식도 많이 보세요."
    ],
    goal: "불가능을 대안 제시 구조로 전환",
    nextOptions: [
      { id: "job_urgent", label: "빨리 시작해야 하는 상황" },
      { id: "remote_class_info", label: "비대면 수업 안내" },
      { id: "visit_soft_close", label: "대안 확인 위해 방문 제안" },
      { id: "remote_soft_close", label: "전화 상담 제안" }
    ]
  },
  ...
  objection_support_only: {
    ...
    scripts: [
      ...
      "다만 국비가 된다고 해서 무조건 좋은 선택이 되는 건 아니고, OOO님 상황에서 실제로 적용 가능한지랑 어떤 방식이 제일 유리한지를 같이 봐야 정확합니다."
    ],
    goal: "국비 여부 집착을 정확한 확인 필요로 전환",
    nextOptions: [
  ...
  visit_soft_close: {
    ...
    scripts: [
      ...
      "그래서 부담 없이 20~30분 정도만 시간 내주시면 OOO님 상황에 맞게 방향부터 정리해드릴게요. 평일 쪽이 괜찮으실까요, 아니면 주말 쪽이 더 편하실까요?"
    ],
    goal: "방문을 혜택처럼 느끼게 만들기",
    nextOptions: [
...
const constantKnowledge = {
  ...
  talk_tip_1: "상대가 말한 핵심어를 바로 반복: '아, 국비지원이 가장 궁금하셨군요.'",
  talk_tip_2: "질문 뒤에는 이유를 붙이기: '반 배정이 달라져서 여쭤봐요.'",
  talk_tip_3: "설명 길게 하지 말고 한 번 말한 뒤 멈추기",
  talk_tip_4: "방문 유도는 '오세요'보다 '오시면 정확히 체크해드릴 수 있다'로 표현",
  talk_tip_5: "거리가 멀면 내방만 고집하지 말고 전화상담 + 비대면 + 전화상 접수 대안 제시",
  support_tip_1: "실제 부담액은 개인 상황, 카드 여부, 지원 유형에 따라 달라질 수 있음",
  support_tip_2: "국비가 안 된다고 바로 종료하지 말고 다른 가능 방향과 순서를 같이 안내",
  closing_tip: "예약은 열린 질문보다 선택형 질문이 강함: '평일/주말 중 어느 쪽이 편하세요?'"
};